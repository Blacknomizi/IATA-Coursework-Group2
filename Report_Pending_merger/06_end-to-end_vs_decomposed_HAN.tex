...
To assess downstream usability, we exported the predictions of each approach into a structured JSON format that maps every document to extracted P/I/O texts. This lets us answer simple but realistic queries, for example \emph{“find trials where the intervention was a placebo”} or \emph{“list outcomes reported as mortality”}. Even without a complete evaluation benchmark, being able to easily build these queries from the saved predictions shows that the end-to-end model, which had the highest recall, gives the most useful output for later use. The decomposed pipeline has problems with early filtering mistakes, which makes it harder to find some entities later. This trade-off shows that recall is not just a technical measure but is also important for practical question answering using the extracted table.
\section*{References}
